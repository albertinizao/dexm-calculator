...
@RestController public class ProtectiveCatalogController { private final ProtectiveEquipmentService service; public ProtectiveCatalogController(ProtectiveEquipmentService s){service=s;}
 ...
 @PostMapping("/api/armor-catalog") public ResponseEntity<?> createArmor(@Valid @RequestBody CharacterController.ArmorRequest r){return ResponseEntity.status(HttpStatus.CREATED).body(service.createArmorCatalog(r));}
 ...
 @PostMapping("/api/physical-shield-catalog") public ResponseEntity<?> createPhysicalShield(@Valid @RequestBody CharacterController.PhysicalShieldRequest r){return ResponseEntity.status(HttpStatus.CREATED).body(service.createPhysicalShieldCatalog(r));}
