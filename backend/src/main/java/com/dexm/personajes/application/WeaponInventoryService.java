...
public class WeaponInventoryService {
    ...
    public Map<String,Object> shoot(String characterId, String weaponId, ShootCommand command) {
        ...
        if (isMelee(weapon)) return meleeAttackResult(weapon, command);
    ...
    private int cadence(String rate) {
        ...
        String cadence = value.replaceFirst("^\\D*(\\d+).*$", "$1");
        try {
    ...
    private WeaponEntity from(String id,String characterId,String slot,WeaponRequest r){return new WeaponEntity(id,characterId,slot,r.name().trim(),r.weaponType(),r.size(),r.range(),r.reload(),r.rate(),r.damageVital(),r.damageNormal(),r.damageLight(),r.damageVeryLight(),r.aim(),clean(r.automaticFire()),capacityFor(r),loadedBulletsFor(r),caliberFor(r),clean(r.extraRule()));}
    private void apply(WeaponEntity e,WeaponRequest r){e.setName(r.name().trim());e.setWeaponType(r.weaponType());e.setSize(r.size());e.setRange(r.range());e.setReload(r.reload());e.setRate(r.rate());e.setDamageVital(r.damageVital());e.setDamageNormal(r.damageNormal());e.setDamageLight(r.damageLight());e.setDamageVeryLight(r.damageVeryLight());e.setAim(r.aim());e.setAutomaticFire(clean(r.automaticFire()));e.setCapacity(capacityFor(r));e.setLoadedBullets(loadedBulletsFor(r));e.setCaliber(caliberFor(r));e.setExtraRule(clean(r.extraRule()));}
    ...
    private void ensureValidRequest(WeaponRequest r){if(r==null||!TYPES.contains(r.weaponType())||!SIZES.contains(r.size()))throw new IllegalArgumentException("Tipo o tamaño de arma no válido"); ensureCompatible(r.size(),normalizeSlot(r.slot())); if(isMeleeType(r.weaponType())) return; integralPositiveCapacity(r.capacity()); loadedBullets(r.loadedBullets(),r.capacity()); if(r.caliber()==null || r.caliber().isBlank()) throw new IllegalArgumentException("El calibre es obligatorio para armas de fuego");}
    ...
    private boolean isMelee(WeaponEntity weapon){return isMeleeType(weapon.getWeaponType());}
    private boolean isMeleeType(String type){return type!=null && type.startsWith("CUERPO_");}
    private BigDecimal capacityFor(WeaponRequest r){return isMeleeType(r.weaponType()) ? BigDecimal.ONE : r.capacity();}
    private BigDecimal loadedBulletsFor(WeaponRequest r){return isMeleeType(r.weaponType()) ? BigDecimal.ZERO : loadedBullets(r.loadedBullets(),r.capacity());}
    private String caliberFor(WeaponRequest r){return isMeleeType(r.weaponType()) ? null : clean(r.caliber());}
    private Map<String,Object> meleeAttackResult(WeaponEntity weapon, ShootCommand command){int requested=positiveShots(command.shots());int cadence=cadence(weapon.getRate());if(requested>cadence)throw new IllegalArgumentException("No se pueden realizar más ataques que la cadencia del arma");var result=new LinkedHashMap<String,Object>();result.put("weaponId",weapon.getId());result.put("mode","melee");result.put("requested",requested);result.put("consumed",0);result.put("remaining",0);result.put("cadence",cadence);result.put("automaticFire",null);result.put("loadedBullets",0);return result;}
