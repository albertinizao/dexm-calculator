...
const armorDraft = ref<Omit<Armor, 'id'>>({ name:'', description:'', specialEffect:'', defensePenalty:0, meleeDefensePenalty:0, rangedDefensePenalty:0, movementPenalty:0, slots:[], rdBySlot:{HEAD:0,BODY:0,LEGS:0,ARMS:0}, armorBySlot:{HEAD:0,BODY:0,LEGS:0,ARMS:0}, imageUrl:null });
const shieldDraft = ref<Omit<Shield, 'id'>>({ name:'', description:'', hitPoints:0, imageUrl:null });
const physicalShieldDraft = ref<Omit<PhysicalShield, 'id'>>({ name:'', description:'', rd:0, armor:0, defense:0, movement:0, size:'', otherEffects:'', imageUrl:null });
const catalogArmors = ref<ArmorCatalogItem[]>([]); const catalogShields = ref<ShieldCatalogItem[]>([]); const catalogPhysicalShields = ref<PhysicalShieldCatalogItem[]>([]);
...
const selectedCatalogArmor = ref<ArmorCatalogItem | null>(null); const selectedCatalogShield = ref<ShieldCatalogItem | null>(null); const selectedCatalogPhysicalShield = ref<PhysicalShieldCatalogItem | null>(null);
const showCatalogArmorModal = ref(false); const showCatalogShieldModal = ref(false); const showCatalogPhysicalShieldModal = ref(false);
...
const compatibleCatalogArmors = computed(() => {
  const requestedSlot = armorDraft.value.slots[0];
  return catalogArmors.value.filter(item =>
    (!requestedSlot || item.slots.includes(requestedSlot)) &&
    item.slots.every(slot => !occupiedArmorSlots.value.has(slot)),
  );
});
...
const transferAction = ref<'import' | 'export'>('export');
const transferMode = ref<'actual' | 'legacy'>('actual');
...
const trainingEndAgeOptions = computed(() => {
  ...
  const maximumEndAge = trainingDraft.value.type === 'FORMATION'
    ? Math.min(trainingData.value.sheetAge + 1, startAge + maximumFormationDuration)
    : trainingData.value.sheetAge + 1;
...
const trainingStartAgeOptions = computed(() => trainingAgeOptions.value.filter(startAge => Array.from({ length: Math.max(0, trainingData.value.sheetAge - startAge + 2) }, (_, index) => startAge + index + 1).some(endAge => trainingIntervalAllowed(startAge, endAge))));
...
function openProfileModal() {
  if (character.value) profileImageDraft.value = character.value.imageUrl ?? null;
...
function closeProfileModal() {
  if (profileImageProcessing.value) return;
  showProfileModal.value = false;
}
async function onProfileImage(event: Event) {
  const file = (event.target as HTMLInputElement).files?.[0];
  if (!file) return;
  const token = ++profileImageProcessingToken;
  profileImageProcessing.value = true;
  profileError.value = '';
  try {
    const processed = await processPortraitFile(file);
    if (token === profileImageProcessingToken) {
      profileImageDraft.value = processed;
      profileImageChanged.value = true;
    }
  } catch (e: any) {
    if (token === profileImageProcessingToken) profileError.value = e?.message || 'No se pudo procesar la imagen.';
  } finally {
    if (token === profileImageProcessingToken) profileImageProcessing.value = false;
  }
}
function clearProfileImage() {
  profileImageDraft.value = null;
  profileImageChanged.value = true;
  profileError.value = '';
  if (profileImageInput.value) profileImageInput.value.value = '';
}
...
function openWeaponAimRolls(weapon: Weapon, rollCount: number) {
  ...
  const details = attributeRollDetails(attributeKey, character.value?.attributeTotals?.[attributeKey] ?? attributes.value[attributeKey] ?? 0, false);
  ...
  weaponAimRoll.value = {
    ...
    attributeName: melee ? 'Destreza' : 'Puntería',
    isMelee: melee,
    weaponAim,
    damage: {
...
const currentSection = computed<keyof typeof characterRouteNames>(() => {
  if (route.name === 'character-abilities') return 'abilities';
  if (route.name === 'character-inventory') return 'inventory';
  return 'sheet';
});
...
function emptyArmorDraft(slot?:ArmorSlot):Omit<Armor,'id'>{return {name:'',description:'',specialEffect:'',defensePenalty:0,meleeDefensePenalty:0,rangedDefensePenalty:0,movementPenalty:0,zone:'',slots:slot?[slot]:[],rdBySlot:{HEAD:0,BODY:0,LEGS:0,ARMS:0},armorBySlot:{HEAD:0,BODY:0,LEGS:0,ARMS:0},imageUrl:null};}
function openArmorDetail(a?:Armor, slot?:ArmorSlot){ selectedArmor.value=a||null; armorDraft.value=a?{...a,slots:[...a.slots],rdBySlot:{...a.rdBySlot},armorBySlot:{...a.armorBySlot}}:emptyArmorDraft(slot); if(a){showArmorDetailModal.value=true;sheetView.value='inventory';}else{openArmorChoice(slot);} }
function openArmorChoice(slot?:ArmorSlot){selectedArmor.value=null;armorDraft.value=emptyArmorDraft(slot);sheetView.value='armor-choice';}
...
function emptyPhysicalShieldDraft():Omit<PhysicalShield,'id'>{return {name:'',description:'',rd:0,armor:0,defense:0,movement:0,size:'',otherEffects:'',imageUrl:null};}
function openPhysicalShieldDetail(s?:PhysicalShield){selectedPhysicalShield.value=s||null;physicalShieldDraft.value=s?{...s}:emptyPhysicalShieldDraft();if(s){showPhysicalShieldDetailModal.value=true;sheetView.value='inventory';}else{sheetView.value='shield-choice';}}
function openPhysicalShieldChoice(){selectedPhysicalShield.value=null;physicalShieldDraft.value=emptyPhysicalShieldDraft();sheetView.value='shield-choice';}
...
function armorRd(item:Armor|ArmorCatalogItem, slot:ArmorSlot){ return item.slots.includes(slot) ? (item.rdBySlot[slot] ?? 0) : 0; }
function armorPenaltyText(item:Armor|ArmorCatalogItem){ const parts:string[]=[]; const format=(value:number)=>String(value).replace('.',','); if(Number(item.defensePenalty||0)>0) parts.push(`-${format(Number(item.defensePenalty))} Defensa`); if(Number(item.meleeDefensePenalty||0)>0) parts.push(`-${format(Number(item.meleeDefensePenalty))} Defensa cuerpo a cuerpo`); if(Number(item.rangedDefensePenalty||0)>0) parts.push(`-${format(Number(item.rangedDefensePenalty))} Defensa a distancia`); if(Number(item.movementPenalty||0)>0) parts.push(`Movimiento -${format(Number(item.movementPenalty))}`); if(item.specialEffect) parts.push(item.specialEffect); return parts.join('; ') || '—'; }
...
function shootOptions(weapon: Weapon){
  const maximum=isMeleeWeapon(weapon) ? weaponCadence(weapon) : Math.min(weaponCadence(weapon), weapon.loadedBullets);
  const options=Array.from({length: maximum}, (_, index) => ({label:`${index + 1} ${isMeleeWeapon(weapon) ? 'ataque' : 'disparo'}${index ? 's' : ''}`, shots:index + 1, automatic:false}));
...
async function loadWeaponCatalog(){ catalogLoading.value=true; inventoryError.value=''; try { const catalog=await loadStaticObjects(); const sizes=catalogSlot.value.startsWith('SMALL')?['PEQUENA']:catalogSlot.value.startsWith('MEDIUM')?['PEQUENA','MEDIANA']:weaponSizes.map(size=>size.value); catalogWeapons.value=catalog.weapons.filter(item=>sizes.includes(item.size) && (!catalogType.value || item.weaponType===catalogType.value)); } catch(e:any) { catalogWeapons.value=[]; inventoryError.value=e?.message || 'No se pudo cargar el catálogo estático de armas.'; } finally { catalogLoading.value=false; } }
...
function openShoot(weapon: Weapon){ if(!isMeleeWeapon(weapon) && weapon.loadedBullets <= 0) return; if(weaponCadence(weapon) <= 1){ shootWeapon(weapon, 1); return; } shootWeaponTarget.value=weapon; showShootModal.value=true; inventoryError.value=''; }
...
const profileImageDraft = ref<string | null>(null);
const profileImageChanged = ref(false);
const profileImageProcessing = ref(false);
let profileImageProcessingToken = 0;
...
function localAttributeDetail(key: string): AttributeDetail {
  ...
  if (derivedStatKeys.includes(key)) {
    const derived = derivedStats.value[key];
    const total = displayedAttributeTotal(key, derived?.total ?? derivedBaseValue(key));
    return {
      key,
      name: derived?.name ?? attributeLabels[key] ?? key,
      type: 'DERIVED',
      total,
      ranks: 0,
      maxRanks: null,
      formula: derived?.formula ?? '',
      calculatedValue: derivedBaseValue(key),
      plusOne: 0,
      plusD6: 0,
      modifiers: editing.value
        ? [
...
function detailTotal(): number {
  if (!attributeDetail.value) return 0;
  if (attributeDetail.value.type === 'DERIVED') return displayedAttributeTotal(attributeDetail.value.key, derivedBaseValue(attributeDetail.value.key));
...
function defenseDodgeBonus(): number {
  const value = displayedAttributeTotal('esquiva', attributes.value.esquiva ?? 0);
  return oneBonus('esquiva', value, false);
}
function defenseDexterityBonus(key: string): number {
  if (key !== 'defensaCuerpo') return 0;
  const value = displayedAttributeTotal('destreza', attributes.value.destreza ?? 0);
  return oneBonus('destreza', value, false);
}
function defenseArmorPenalties(key: string) {
  const kind = key === 'defensaCuerpo' ? 'melee' : 'ranged';
  return armors.value.map(armor => ({
    name: armor.name,
    value: (Number(armor.defensePenalty) || 0)
      + (kind === 'melee' ? Number(armor.meleeDefensePenalty) || 0 : Number(armor.rangedDefensePenalty) || 0),
  })).filter(item => item.value > 0);
}
function defenseShieldBonus(): number {
  return physicalShields.value.reduce((total, shield) => total + (Number(shield.defense) || 0), 0);
}

function equippedDefenseAdjustment(kind: 'melee' | 'ranged'): number {
  const armorPenalty = armors.value.reduce((total, armor) => {
    const generalPenalty = Number(armor.defensePenalty) || 0;
    const specificPenalty = kind === 'melee'
      ? Number(armor.meleeDefensePenalty) || 0
      : Number(armor.rangedDefensePenalty) || 0;
    return total + generalPenalty + specificPenalty;
  }, 0);
  const physicalShieldBonus = physicalShields.value.reduce(
    (total, shield) => total + (Number(shield.defense) || 0),
    0,
  );
  return physicalShieldBonus - armorPenalty;
}
...
function derivedBaseValue(key: string): number {
  ...
  if (key === 'defensaCuerpo') {
    return 10 + esquivaPlusOne + oneBonus('destreza', valueOf('destreza'), false)
      + equippedDefenseAdjustment('melee');
  }
  if (key === 'defensaDistancia') return 15 + esquivaPlusOne + equippedDefenseAdjustment('ranged');
  return 0;
}
...
function displayedAttributeTotal(key: string, fallback: number): number {
  ...
  if (!editing.value) {
    if (key === 'defensaCuerpo' || key === 'defensaDistancia') {
      // The derived base already includes the equipped armor and shield.
      // Reusing the persisted aggregate here applied the equipment penalty a
      // second time when the backend aggregate already contained it.
      return derivedBaseValue(key) + modifierTotal(character.value?.attributeModifiers?.[key] ?? []);
    }
...
function downloadArchive(archive: unknown) { const name = character.value?.name || 'personaje'; const url = URL.createObjectURL(new Blob([JSON.stringify(archive, null, 2)], { type: 'application/json' })); const link = document.createElement('a'); link.href = url; link.download = `${name}.dexm.json`; link.click(); URL.revokeObjectURL(url); }
async function exportArchive() { try { downloadArchive(await api.exportCharacterArchive(String(route.params.id))); } catch (e: any) { error.value = e?.message || 'No se pudo exportar el personaje.'; } }
function openTransferModal() { transferAction.value = 'export'; transferMode.value = 'actual'; legacyCode.value = ''; legacyError.value = ''; showTransferModal.value = true; }
function chooseTransferFile() { archiveInput.value?.click(); }
async function executeTransfer() {
  if (transferMode.value === 'legacy') { if (transferAction.value === 'export') { showTransferModal.value = false; await exportLegacy(); } else { showTransferModal.value = false; showLegacyImport.value = true; legacyError.value = ''; } return; }
  if (transferAction.value === 'export') { showTransferModal.value = false; await exportArchive(); } else chooseTransferFile();
}
function chooseArchive() { openTransferModal(); }
async function importArchive(event: Event) { const file = (event.target as HTMLInputElement).files?.[0]; if (!file) return; showTransferModal.value = false; if (!confirm('Se reemplazarán los datos de juego de este personaje. Su retrato y permisos se conservarán. ¿Continuar?')) return; try { await api.importCharacterArchive(String(route.params.id), JSON.parse(await file.text())); await load(); } catch (e: any) { error.value = e?.message || 'No se pudo importar el archivo.'; } finally { if (archiveInput.value) archiveInput.value.value = ''; } }

async function closeDraft() {
  if (!character.value || !editing.value || closeBusy.value || profileImageProcessing.value) return;
...
watch(() => route.name, async (name, previousName) => {
    ...
    <div v-else-if="character" class="sheet-layout">
      ...
      <aside class="sheet-sidebar">
        ...
        <p v-if="closeError" class="error-banner" role="alert">{{ closeError }}</p><input ref="archiveInput" class="sr-only" type="file" accept="application/json,.dexm.json" @change="importArchive">
         ...
         <button v-if="editing" class="button button-primary" :disabled="closeBusy || levelBusy || modifierSaveBusy || profileImageProcessing" @click="closeDraft">{{ profileImageProcessing ? 'Procesando retrato…' : (closeBusy ? 'Guardando…' : 'Guardar ficha') }}</button><button v-if="editing" class="button button-quiet" type="button" :disabled="cancelChangesBusy || profileImageProcessing" @click="cancelChanges">{{ cancelChangesBusy ? 'Cancelando…' : 'Cancelar subida' }}</button><button v-else-if="canEdit" class="button button-quiet" type="button" @click="startEdit">Editar ficha</button><button class="button button-quiet" type="button" :disabled="showProfileModal" @click="openProfileModal">Perfil</button><button class="button button-quiet" type="button" @click="editing ? openCurrentUpgrade() : openLastUpgrade()">Última subida</button><button class="button button-quiet" type="button" @click="openHistory">Historial</button><button v-if="canEdit" class="button button-quiet" type="button" @click="openTransferModal">Exportar/Importar</button><button v-if="props.isDirector && pendingUniqueAbilities.length" class="button button-primary unique-review-trigger" type="button" @click="openUniqueReview">Revisar habilidades únicas</button><button class="button button-quiet" type="button" :disabled="profileImageProcessing" @click="back">Salir</button><button v-if="props.isDirector" class="button button-quiet" type="button" @click="openEditorsModal">Permisos</button><button v-if="props.isDirector" class="button button-danger" type="button" @click="deleteCharacter">Borrar Personaje</button>
       ...
       <nav class="sheet-tabs" aria-label="Secciones de la ficha">
         <button class="sheet-tab" :class="{ active: route.name === 'character-sheet' || route.name === 'character-history' }" type="button" :aria-current="route.name === 'character-sheet' || route.name === 'character-history' ? 'page' : undefined" @click="navigateToSection('sheet')">Ficha</button>
         <button class="sheet-tab" :class="{ active: route.name === 'character-abilities' }" type="button" :aria-current="route.name === 'character-abilities' ? 'page' : undefined" @click="navigateToSection('abilities')">Habilidades</button>
         <button class="sheet-tab" :class="{ active: route.name === 'character-inventory' }" type="button" :aria-current="route.name === 'character-inventory' ? 'page' : undefined" @click="navigateToSection('inventory')">Inventario</button>
       </nav>
       ...
       <div v-if="showEditorsModal" class="modal-backdrop" @click.self="showEditorsModal=false">
        <section class="modal-card creation-modal" role="dialog" aria-modal="true" aria-labelledby="editors-modal-title">
          <header class="modal-header"><div><p class="eyebrow accent">ACCESO DEL PERSONAJE</p><h2 id="editors-modal-title">Permisos</h2><p class="modal-copy">Solo los miembros de la campaña añadidos aquí podrán modificar esta ficha.</p></div><button class="modal-close" type="button" aria-label="Cerrar ventana" @click="showEditorsModal=false">×</button></header>
      ...
      <div v-if="showProfileModal" class="modal-backdrop" @click.self="closeProfileModal">
        <section class="modal-card profile-modal" role="dialog" aria-modal="true" aria-labelledby="profile-modal-title">
          <header class="modal-header"><div><p class="eyebrow accent">PERSONAJE · CONFIGURACIÓN</p><h2 id="profile-modal-title">Perfil Einherjer</h2><p class="modal-copy">{{ editing ? 'Actualiza la edad y los datos de origen del personaje.' : 'Consulta la edad y los datos de origen del personaje.' }}</p></div><button class="modal-close" type="button" aria-label="Cerrar ventana" :disabled="profileImageProcessing" @click="closeProfileModal">×</button></header>
          <div v-if="!editing" class="modal-body profile-summary profile-summary-modal"><span>Edad actual<strong>{{ character.sheetAge ?? 'No indicada' }}</strong></span><span>Origen<strong>{{ originLabel(character.einherjerOrigin) }}</strong></span><span>Despertado<strong>{{ character.awakened ? 'Sí' : 'No' }}</strong></span><span v-if="character.awakened && character.awakeningAge !== null">Edad de despertar<strong>{{ character.awakeningAge }}</strong></span></div>
           <div v-else class="modal-body profile-modal-form">
             <section class="modal-section-panel profile-image-section"><h3 class="modal-section-title">Retrato</h3><div class="profile-image-editor"><div class="profile-image-preview"><img v-if="profileImageDraft" :src="profileImageDraft" :alt="`Vista previa de ${character.name}`" /><span v-else>{{ initials(character.name) }}</span></div><div class="profile-image-controls"><label class="file-input"><span>Subir retrato</span><input ref="profileImageInput" type="file" accept="image/jpeg,image/png,image/webp" :disabled="profileImageProcessing" @change="onProfileImage" /></label><button v-if="profileImageDraft" class="button button-quiet" type="button" :disabled="profileImageProcessing" @click="clearProfileImage">Quitar imagen</button><p v-if="profileImageProcessing" class="field-hint" role="status">Procesando retrato…</p><p v-else class="field-hint">Se comprime en el navegador a JPEG y se limita a 150 KB.</p></div></div><p v-if="profileError" class="error-banner" role="alert">{{ profileError }}</p></section>
             <section class="modal-section-panel"><h3 class="modal-section-title">Datos del personaje</h3><div class="profile-form-grid"><label class="modal-field"><span>Edad inicial</span><input v-model.number="character.startingAge" type="number" min="0" :max="character.sheetAge ?? undefined" required :disabled="profileImageProcessing" /></label><label class="modal-field"><span>Edad actual</span><input v-model.number="character.sheetAge" type="number" :min="(character.startingAge ?? 0) + 1" required :disabled="profileImageProcessing" /></label><label class="modal-field"><span>Origen</span><select v-model="character.einherjerOrigin" required :disabled="profileImageProcessing"><option :value="null" disabled>Selecciona un origen</option><option value="converted">Convertido</option><option value="born_human">Nacido de humanos</option><option value="born_einherjer">Nacido de Einherjer</option></select></label></div></section>
            <section class="modal-section-panel"><h3 class="modal-section-title">Despertar</h3><div class="profile-form-grid"><div class="profile-awakening-field"><span class="field-label">¿Ha despertado?</span><div class="guided-answer-grid"><button class="answer-card" type="button" :class="{ selected: character.awakened === false }" :disabled="profileImageProcessing" @click="setAwakened(false)">No</button><button class="answer-card" type="button" :class="{ selected: character.awakened === true }" :disabled="profileImageProcessing" @click="setAwakened(true)">Sí</button></div></div><label v-if="character.awakened" class="modal-field"><span>Edad de despertar / conversión</span><input v-model.number="character.awakeningAge" type="number" :min="character.startingAge ?? 0" :max="character.sheetAge ?? undefined" required :disabled="profileImageProcessing" /></label></div></section>
          </div>
          <footer class="modal-actions"><button class="button button-primary" type="button" @click="editing ? closeDraft() : closeProfileModal()" :disabled="profileImageProcessing || (editing && closeBusy)">{{ editing ? (profileImageProcessing ? 'Procesando retrato…' : (closeBusy ? 'Guardando…' : 'Guardar perfil')) : 'Cerrar' }}</button></footer>
        </section>
      </div>
      ...
      <div v-if="showTransferModal" class="modal-backdrop" @click.self="showTransferModal=false">
        <section class="modal-card transfer-modal" role="dialog" aria-modal="true" aria-labelledby="transfer-title">
          <header class="modal-header"><div><p class="eyebrow accent">COPIA DE SEGURIDAD</p><h2 id="transfer-title">Exportar / importar personaje</h2><p class="modal-copy">Elige una operación y el formato compatible con tu ficha.</p></div><button class="modal-close" type="button" aria-label="Cerrar ventana" @click="showTransferModal=false">×</button></header>
          <div class="modal-body transfer-body">
            <div class="transfer-choice-group" role="group" aria-label="Operación"><button type="button" class="transfer-choice" :class="{ selected: transferAction === 'import' }" :disabled="!canEdit" @click="transferAction='import'">Importar</button><button type="button" class="transfer-choice" :class="{ selected: transferAction === 'export' }" @click="transferAction='export'">Exportar</button></div>
            <div class="transfer-mode-row"><span class="field-label">Modo</span><button type="button" class="transfer-toggle" :class="{ active: transferMode === 'actual' }" :aria-pressed="transferMode === 'actual'" @click="transferMode = transferMode === 'actual' ? 'legacy' : 'actual'"><span>Legacy</span><span class="transfer-toggle-track"><i></i></span><span>Actual</span></button></div>
            <p v-if="transferMode === 'actual'" class="modal-copy">Formato JSON actual: contiene todos los datos de juego y nunca incluye imágenes ni permisos.</p><p v-else class="modal-copy">Formato Legacy: compatible con el código de backup de la ficha HTML antigua.</p>
          </div>
          <footer class="modal-actions"><button class="button button-quiet" type="button" @click="showTransferModal=false">Cancelar</button><button class="button button-primary" type="button" @click="executeTransfer">{{ transferAction === 'import' ? (transferMode === 'actual' ? 'Seleccionar fichero' : 'Cargar legacy') : (transferMode === 'actual' ? 'Descargar fichero' : 'Mostrar código') }}</button></footer>
        </section>
      </div>
        ...
        <section class="modal-card attribute-detail-modal" role="dialog" aria-modal="true" aria-labelledby="attribute-detail-title">
          ...
          <div class="modal-body" v-else-if="attributeDetail">
            <p v-if="detailError" class="error-banner" role="alert">{{ detailError }}</p>
             <template v-if="attributeDetail.type === 'DERIVED' && (attributeDetail.key === 'defensaCuerpo' || attributeDetail.key === 'defensaDistancia')">
               <div class="attribute-detail-summary defense-detail-summary"><div><span>Total de defensa</span><strong>{{ detailTotal() }}</strong></div></div>
               <div class="detail-grid defense-detail-grid">
                 <p><span>Base</span><strong>{{ attributeDetail.key === 'defensaCuerpo' ? 10 : 15 }}</strong></p>
                 <p><span>Bonificador de esquiva</span><strong>+{{ defenseDodgeBonus() }}</strong></p>
                 <p v-if="attributeDetail.key === 'defensaCuerpo'"><span>Bonificador de destreza</span><strong>+{{ defenseDexterityBonus(attributeDetail.key) }}</strong></p>
                 <p><span>Bonificador del escudo físico</span><strong>+{{ defenseShieldBonus() }}</strong></p>
               </div>
               <section class="detail-section"><h3>Penalizadores de armadura</h3><ul v-if="defenseArmorPenalties(attributeDetail.key).length" class="modifier-list"><li v-for="penalty in defenseArmorPenalties(attributeDetail.key)" :key="penalty.name"><span>{{ penalty.name }}</span><strong>-{{ penalty.value }}</strong></li></ul><p v-else class="sheet-muted">Sin penalizadores de armadura.</p></section>
               <section class="detail-section"><h3>Modificadores varios</h3><ul v-if="attributeDetail.modifiers.length" class="modifier-list"><li v-for="modifier in attributeDetail.modifiers" :key="modifier.name"><span>{{ modifierDisplayName(modifier.name) }}</span><strong>{{ modifier.value > 0 ? '+' : '' }}{{ modifier.value }}</strong></li></ul><p v-else class="sheet-muted">Sin modificadores varios.</p></section>
             </template>
              <template v-else-if="attributeDetail.type !== 'DERIVED'">
              ...
              <template v-else>
                <div class="attribute-detail-summary"><div><span>Valor total</span><strong>{{ detailTotal() }}</strong></div></div>
                <div class="detail-grid"><p class="detail-wide"><span>Fórmula</span><strong>{{ attributeDetail.formula }} = {{ attributeDetail.calculatedValue }}</strong></p></div>
                <section class="detail-section"><h3>Modificadores varios</h3><ul v-if="attributeDetail.modifiers.length" class="modifier-list"><li v-for="modifier in attributeDetail.modifiers" :key="modifier.name"><span>{{ modifierDisplayName(modifier.name) }}</span><strong>{{ modifier.value > 0 ? '+' : '' }}{{ modifier.value }}</strong></li></ul><p v-else class="sheet-muted">Sin modificadores varios.</p></section>
              </template>
...
<section class="sheet-panel"><h3>Atributos Mayores</h3><div class="value-grid attributes-grid major-attributes-grid"><div v-for="[key, value] in majorAttributes" :key="key" class="attribute-card"><button class="value-row attribute-row attribute-clickable attribute-detail-trigger" type="button" @click="openAttributeDetail(key)"><span>{{ attributeLabels[key] || key }}</span><strong>{{ displayedAttributeTotal(key, value) }}</strong></button><div class="attribute-card-footer"><button class="attribute-roll-trigger" type="button" :aria-label="`Tirar D10 de ${attributeLabels[key] || key}`" @click="openAttributeRoll(key, value, true)"><span class="attribute-roll-icon" aria-hidden="true"><img src="/diceD10.png" alt=""></span></button><small class="attribute-bonus">+{{ oneBonus(key, displayedAttributeTotal(key, value), true) }} · +{{ d6Bonus(key, displayedAttributeTotal(key, value), true) }}D6</small></div></div><p v-if="!Object.keys(attributes).length" class="sheet-muted">Sin atributos registrados.</p></div></section>
       ...
       <section v-else-if="sheetView === 'inventory'" class="sheet-content inventory-content" aria-labelledby="inventory-title">
         <div class="sheet-heading"><div><h2 id="inventory-title">Inventario</h2><p class="modal-copy">Armas equipadas y otros objetos personales.</p></div><button class="button button-primary" type="button" @click="openInventoryType">＋ Añadir objeto</button></div>
        ...
        <section class="weapon-section protection-section"><div class="sheet-panel-heading"><h3>Protecciones</h3></div><div class="weapon-slots protection-slots">
          ...
          <article class="weapon-slot protection-slot"><header><strong>Escudo</strong><button v-if="!physicalShields.length" class="button button-quiet" type="button" @click="openPhysicalShieldDetail()">Añadir</button></header><template v-if="physicalShields.length"><button class="inventory-item weapon-card shield-card" type="button" @click="openPhysicalShieldDetail(physicalShields[0])"><img v-if="physicalShields[0].imageUrl" class="weapon-thumbnail" :src="physicalShields[0].imageUrl" :alt="physicalShields[0].name"><span><strong>{{ physicalShields[0].name }}</strong><small>Escudo físico activo</small><small class="weapon-card-stats">RD {{ physicalShields[0].rd || '—' }} · Defensa +{{ physicalShields[0].defense }} · Movimiento {{ physicalShields[0].movement || '—' }}</small></span></button></template><p v-else class="sheet-muted">Hueco libre</p></article>
      ...
      <section v-else-if="sheetView === 'armor-choice'" class="sheet-content inventory-content"><div class="sheet-heading"><div><p class="eyebrow accent">INVENTARIO · ARMADURA</p><h2>Añadir armadura</h2><p class="modal-copy">Elige una pieza oficial del catálogo o crea una armadura personalizada.</p></div></div><div class="inventory-type-grid"><button class="inventory-type-card" type="button" @click="openArmorCatalog"><strong>Del catálogo</strong><span>Consulta las piezas oficiales con sus valores y efectos.</span></button><button class="inventory-type-card" type="button" @click="sheetView='armor-detail'"><strong>Personalizada</strong><span>Define manualmente la zona, RD y Armadura.</span></button></div><div class="modal-actions"><button class="button button-quiet" type="button" @click="sheetView='inventory'">Cancelar</button></div></section>

      <section v-else-if="sheetView === 'shield-choice'" class="sheet-content inventory-content"><div class="sheet-heading"><div><p class="eyebrow accent">INVENTARIO · ESCUDO FÍSICO</p><h2>Añadir escudo físico</h2><p class="modal-copy">Elige un escudo oficial del catálogo o crea uno personalizado.</p></div></div><div class="inventory-type-grid"><button class="inventory-type-card" type="button" @click="openPhysicalShieldCatalog"><strong>Del catálogo</strong><span>Consulta los escudos oficiales con sus valores y efectos.</span></button><button class="inventory-type-card" type="button" @click="sheetView='physical-shield-detail'"><strong>Personalizado</strong><span>Define manualmente RD, Armadura, Defensa y efectos.</span></button></div><div class="modal-actions"><button class="button button-quiet" type="button" @click="sheetView='inventory'">Cancelar</button></div></section>

      <section v-else-if="sheetView === 'armor-catalog'" class="sheet-content inventory-content"><div class="sheet-heading"><div><p class="eyebrow accent">INVENTARIO · ARMADURA</p><h2>Catálogo de armaduras</h2><p class="modal-copy">Solo se muestran piezas compatibles con el hueco seleccionado y con los demás huecos libres.</p></div></div><p v-if="inventoryError" class="error-banner" role="alert">{{ inventoryError }}</p><p v-if="armorCatalogLoading" class="sheet-state">Cargando armaduras…</p><div v-else class="character-grid weapon-catalog-grid protective-catalog-grid"><button v-for="item in compatibleCatalogArmors" :key="item.id" class="weapon-catalog-card character-card" type="button" @click="selectCatalogArmor(item)"><div class="portrait weapon-portrait protective-portrait"><img v-if="item.imageUrl" :src="item.imageUrl" :alt="item.name"><span v-else>🛡</span></div><div class="character-info weapon-catalog-info"><p class="eyebrow accent">{{ item.zone || 'ARMADURA' }}</p><h3>{{ item.name }}</h3><small class="weapon-catalog-type">{{ armorPenaltyText(item) }}</small><dl class="weapon-catalog-stats"><div><dt>Vital</dt><dd>{{ armorValue(item,'HEAD') }}</dd></div><div><dt>Cuerpo</dt><dd>{{ armorValue(item,'BODY') }}</dd></div><div><dt>Ligero</dt><dd>{{ armorValue(item,'LEGS') }}</dd></div><div><dt>Muy ligero</dt><dd>{{ armorValue(item,'ARMS') }}</dd></div><div><dt>RD</dt><dd>{{ Math.max(armorRd(item,'HEAD'),armorRd(item,'BODY'),armorRd(item,'LEGS'),armorRd(item,'ARMS')) || '—' }}</dd></div></dl></div></button></div><p v-if="!armorCatalogLoading && !compatibleCatalogArmors.length" class="sheet-state">No hay armaduras compatibles con este hueco y los huecos libres.</p><div class="modal-actions"><button class="button button-quiet" type="button" @click="sheetView='armor-choice'">Volver</button></div></section>

      <section v-else-if="sheetView === 'shield-catalog'" class="sheet-content inventory-content"><div class="sheet-heading"><div><p class="eyebrow accent">INVENTARIO · ESCUDO FÍSICO</p><h2>Catálogo de escudos físicos</h2><p class="modal-copy">Selecciona un escudo para revisar sus detalles antes de añadirlo.</p></div></div><p v-if="inventoryError" class="error-banner" role="alert">{{ inventoryError }}</p><p v-if="shieldCatalogLoading" class="sheet-state">Cargando escudos…</p><div v-else class="character-grid weapon-catalog-grid protective-catalog-grid"><button v-for="item in catalogPhysicalShields" :key="item.id" class="weapon-catalog-card character-card" type="button" @click="selectCatalogPhysicalShield(item)"><div class="portrait weapon-portrait protective-portrait"><img v-if="item.imageUrl" :src="item.imageUrl" :alt="item.name"><span v-else>🛡</span></div><div class="character-info weapon-catalog-info"><p class="eyebrow accent">ESCUDO FÍSICO</p><h3>{{ item.name }}</h3><dl class="weapon-catalog-stats"><div><dt>RD</dt><dd>{{ item.rd || '—' }}</dd></div><div><dt>Defensa</dt><dd>+{{ item.defense }}</dd></div><div><dt>Movimiento</dt><dd>{{ item.movement || '—' }}</dd></div></dl></div></button></div><p v-if="!shieldCatalogLoading && !catalogPhysicalShields.length" class="sheet-state">No hay escudos físicos disponibles.</p><div class="modal-actions"><button class="button button-quiet" type="button" @click="sheetView='shield-choice'">Volver</button></div></section>
      ...
      <section v-else-if="sheetView === 'inventory-detail'" class="sheet-content inventory-content" aria-labelledby="inventory-detail-title">
       ...
       <section v-else-if="sheetView === 'abilities'" class="sheet-content abilities-content" aria-labelledby="abilities-title">
         <div class="sheet-heading"><div><h2 id="abilities-title">Habilidades</h2><p class="modal-copy">Todas las habilidades obtenidas en la última versión cerrada.</p></div><strong class="ability-count">{{ obtainedAbilities.length }}</strong></div>
      ...
      <section v-else-if="sheetView === 'physical-shield-detail'" class="sheet-content inventory-content"><div class="sheet-heading"><div><p class="eyebrow accent">INVENTARIO · ESCUDO</p><h2>{{ selectedPhysicalShield ? 'Editar escudo' : 'Añadir escudo' }}</h2></div></div><p v-if="inventoryError" class="error-banner" role="alert">{{ inventoryError }}</p><form class="inventory-form" @submit.prevent="savePhysicalShield"><label class="modal-field"><span>Nombre</span><input v-model="physicalShieldDraft.name" required maxlength="255" autofocus></label><label class="modal-field"><span>Descripción</span><textarea v-model="physicalShieldDraft.description" rows="3"></textarea></label><div class="inventory-form-grid"><label class="modal-field"><span>Resistencia</span><input v-model.number="physicalShieldDraft.armor" type="number" min="0" max="9999" required></label><label class="modal-field"><span>RD</span><input v-model.number="physicalShieldDraft.rd" type="number" min="0" max="999" required></label><label class="modal-field"><span>Defensa</span><input v-model.number="physicalShieldDraft.defense" type="number" required></label><label class="modal-field"><span>Movimiento</span><input v-model.number="physicalShieldDraft.movement" type="number" max="0" required></label></div><label class="modal-field"><span>Tamaño</span><select v-model="physicalShieldDraft.size"><option value="">Sin especificar</option><option value="Pequeño">Pequeño</option><option value="Mediano">Mediano</option><option value="Grande">Grande</option></select></label><label class="modal-field"><span>Otros efectos</span><textarea v-model="physicalShieldDraft.otherEffects" rows="3"></textarea></label><label class="modal-field"><span>Imagen</span><input type="file" accept="image/png,image/jpeg,image/gif,image/webp" @change="onProtectiveImage($event,'physicalShield')"><img v-if="physicalShieldDraft.imageUrl" class="weapon-image-preview" :src="physicalShieldDraft.imageUrl" alt="Vista previa"></label><div class="modal-actions"><button class="button button-primary" type="submit" :disabled="shieldSaving">{{ shieldSaving ? 'Guardando…' : 'Guardar escudo' }}</button><button class="button button-quiet" type="button" @click="sheetView='inventory'">Cancelar</button><button v-if="selectedPhysicalShield" class="button button-danger" type="button" @click="deletePhysicalShield">Borrar</button></div></form></section>
    </div>
    ...
    <div v-if="showArmorDetailModal && selectedArmor" class="modal-backdrop" @click.self="closeArmorDetailModal">
     ...
     <div v-if="showShootModal && shootWeaponTarget" class="modal-backdrop" @click.self="closeShoot">
       <section class="modal-card shoot-modal" role="dialog" aria-modal="true" aria-labelledby="shoot-modal-title">
         <header class="modal-header"><div><p class="eyebrow accent">INVENTARIO · ARMA</p><h2 id="shoot-modal-title">{{ isMeleeWeapon(shootWeaponTarget) ? 'Atacar' : 'Disparar' }} {{ shootWeaponTarget.name }}</h2><p class="modal-copy">{{ isMeleeWeapon(shootWeaponTarget) ? 'Ataques disponibles según cadencia: ' + weaponRate(shootWeaponTarget.rate) : 'Balas cargadas: ' + shootWeaponTarget.loadedBullets + '/' + shootWeaponTarget.capacity }}</p></div><button class="modal-close" type="button" aria-label="Cerrar ventana" @click="closeShoot">×</button></header>
         <div class="modal-body"><div class="shoot-options"><button v-for="option in shootOptions(shootWeaponTarget)" :key="option.label" class="button button-primary" type="button" :disabled="weaponShooting" @click="shootWeapon(shootWeaponTarget, option.shots, option.automatic)">{{ option.label }}<small>{{ option.shots }} {{ isMeleeWeapon(shootWeaponTarget) ? 'ataque' : 'bala' }}{{ option.shots === 1 ? '' : 's' }}</small></button><p v-if="!shootOptions(shootWeaponTarget).length" class="sheet-muted">No hay ataques disponibles.</p></div><p v-if="inventoryError" class="error-banner" role="alert">{{ inventoryError }}</p></div>
    ...
    <div v-if="showWeaponAimRollModal && weaponAimRoll" class="modal-backdrop weapon-aim-roll-backdrop" @click.self="closeWeaponAimRolls">
      <section class="modal-card weapon-aim-roll-modal" role="dialog" aria-modal="true" aria-labelledby="weapon-aim-roll-title" tabindex="-1" @keydown.esc="closeWeaponAimRolls">
        <header class="modal-header">
          <div>
            <p class="eyebrow accent">TIRADA DE {{ weaponAimRoll.isMelee ? 'ATAQUE' : 'PUNTERÍA' }}</p>
            <h2 id="weapon-aim-roll-title">{{ weaponAimRoll.weaponName }}</h2>
            <p class="modal-copy">{{ weaponAimRoll.attributeName }} {{ weaponAimRoll.score }} · +{{ weaponAimRoll.plusOne }} · +{{ weaponAimRoll.plusD6 }}D6 · arma +{{ weaponAimRoll.weaponAim }}</p>
          </div>
        ...
        <div class="modal-body weapon-aim-roll-body">
          <p class="attribute-roll-help">Cada {{ weaponAimRoll.isMelee ? 'ataque' : 'disparo' }} se tira por separado. Selecciona 1 D10 y 2 D6; la puntería del arma se suma como bonificación adicional.</p>
          <div class="weapon-aim-roll-list">
            <article v-for="(roll, index) in weaponAimRoll.rolls" :key="roll.id" class="weapon-aim-roll-card" :aria-labelledby="`weapon-aim-roll-${roll.id}-title`">
              <header class="weapon-aim-roll-heading">
                <h3 :id="`weapon-aim-roll-${roll.id}-title`">{{ weaponAimRoll.isMelee ? 'Ataque' : 'Disparo' }} {{ index + 1 }}</h3>
                <button class="button button-quiet" type="button" :aria-label="`Volver a tirar el ${weaponAimRoll.isMelee ? 'ataque' : 'disparo'} ${index + 1}`" @click="rerollWeaponAimRoll(roll.id)">Volver a tirar</button>
              </header>
              <div class="weapon-aim-roll-layout">
                <section class="attribute-roll-dice-group" :aria-labelledby="`weapon-aim-roll-${roll.id}-d10-label`">
                  ...
                  <div class="attribute-roll-dice">
                    <button v-for="die in roll.dice.filter(item => item.type === 'd10')" :key="die.id" class="roll-die" :class="{ selected: die.selected }" type="button" :aria-pressed="die.selected" :aria-label="`${weaponAimRoll.isMelee ? 'Ataque' : 'Disparo'} ${index + 1}, D10: ${die.value}${die.selected ? ', seleccionado' : ', no seleccionado'}`" @click="toggleWeaponAimRollDie(roll.id, die.id)">
                ...
                <section class="attribute-roll-dice-group" :aria-labelledby="`weapon-aim-roll-${roll.id}-d6-label`">
                  ...
                  <div class="attribute-roll-dice">
                    <button v-for="die in roll.dice.filter(item => item.type === 'd6')" :key="die.id" class="roll-die" :class="{ selected: die.selected, disabled: die.disabled }" type="button" :disabled="die.disabled" :aria-pressed="die.selected" :aria-label="`${weaponAimRoll.isMelee ? 'Ataque' : 'Disparo'} ${index + 1}, D6: ${die.value}${die.disabled ? ', desactivado por sacar 1' : die.selected ? ', seleccionado' : ', no seleccionado'}`" @click="toggleWeaponAimRollDie(roll.id, die.id)">
                      <img :src="attributeRollDieImage(die)" alt="">
                      <strong>{{ die.value }}</strong>
                    </button>
                  </div>
                ...
                <section class="attribute-roll-dice-group weapon-aim-roll-damage" :aria-labelledby="`weapon-aim-roll-${roll.id}-damage-label`">
                  ...
                  <div class="weapon-aim-roll-damage-content">
                    <div class="roll-die selected weapon-aim-roll-damage-die" role="img" :aria-label="`${weaponAimRoll.isMelee ? 'Ataque' : 'Disparo'} ${index + 1}, D10 de daño: ${roll.damageD10.value}, seleccionado y bloqueado`">
     ...
     <div v-if="showWeaponDetailModal && selectedWeapon" class="modal-backdrop" @click.self="closeWeaponDetail">
      <section class="modal-card weapon-detail-modal" role="dialog" aria-modal="true" aria-labelledby="weapon-detail-modal-title">
        ...
        <div class="modal-body weapon-detail-body">
            <div class="weapon-detail-hero"><div class="weapon-detail-image"><img v-if="weaponImage(selectedWeapon)" :src="weaponImage(selectedWeapon)" :alt="selectedWeapon.name"><span v-else>⚔</span></div><dl class="weapon-detail-stats"><div><dt>Daño</dt><dd>{{ weaponDamage(selectedWeapon) }}</dd></div><div><dt>Puntería</dt><dd>{{ selectedWeapon.aim ?? '—' }}</dd></div><div><dt>Alcance</dt><dd>{{ selectedWeapon.range }}</dd></div><div v-if="!isMeleeWeapon(selectedWeapon)"><dt>Recarga</dt><dd>{{ selectedWeapon.reload }}</dd></div><div><dt>Cadencia</dt><dd>{{ weaponRate(selectedWeapon.rate) }}</dd></div><div v-if="!isMeleeWeapon(selectedWeapon)"><dt>Capacidad</dt><dd>{{ selectedWeapon.capacity }}</dd></div><div v-if="!isMeleeWeapon(selectedWeapon)"><dt>Balas cargadas</dt><dd>{{ selectedWeapon.loadedBullets }}/{{ selectedWeapon.capacity }}</dd></div><div v-if="!isMeleeWeapon(selectedWeapon)"><dt>Calibre</dt><dd>{{ selectedWeapon.caliber || '—' }}</dd></div><div v-if="!isMeleeWeapon(selectedWeapon)"><dt>Balas disponibles</dt><dd>{{ ammunitionForCaliber(selectedWeapon.caliber) }}</dd></div><div v-if="!isMeleeWeapon(selectedWeapon)"><dt>Fuego automático</dt><dd>{{ selectedWeapon.automaticFire || '—' }}</dd></div></dl></div>
           <p v-if="inventoryError" class="error-banner" role="alert">{{ inventoryError }}</p>
    ...
    <div v-if="showCatalogArmorModal && selectedCatalogArmor" class="modal-backdrop" @click.self="showCatalogArmorModal=false"><section class="modal-card weapon-detail-modal" role="dialog" aria-modal="true" aria-labelledby="catalog-armor-detail-title"><header class="modal-header"><div><p class="eyebrow accent">AÑADIR ARMADURA</p><h2 id="catalog-armor-detail-title">{{ selectedCatalogArmor.name }}</h2><p class="modal-copy">{{ selectedCatalogArmor.zone || '' }} · confirma sus valores antes de añadirla.</p></div><button class="modal-close" type="button" aria-label="Cerrar ventana" @click="showCatalogArmorModal=false">×</button></header><div class="modal-body weapon-detail-body"><div class="weapon-detail-hero"><div class="weapon-detail-image armor-detail-image"><img v-if="selectedCatalogArmor.imageUrl" :src="selectedCatalogArmor.imageUrl" :alt="selectedCatalogArmor.name"><span v-else>🛡</span></div><dl class="weapon-detail-stats"><div><dt>Vital</dt><dd>{{ armorValue(selectedCatalogArmor,'HEAD') }}</dd></div><div><dt>Cuerpo</dt><dd>{{ armorValue(selectedCatalogArmor,'BODY') }}</dd></div><div><dt>Ligero</dt><dd>{{ armorValue(selectedCatalogArmor,'LEGS') }}</dd></div><div><dt>Muy ligero</dt><dd>{{ armorValue(selectedCatalogArmor,'ARMS') }}</dd></div><div><dt>RD</dt><dd>{{ Math.max(armorRd(selectedCatalogArmor,'HEAD'),armorRd(selectedCatalogArmor,'BODY'),armorRd(selectedCatalogArmor,'LEGS'),armorRd(selectedCatalogArmor,'ARMS')) || '—' }}</dd></div></dl></div><p v-if="selectedCatalogArmor.description" class="weapon-detail-rule">{{ selectedCatalogArmor.description }}</p><p class="weapon-detail-rule"><strong>Penalizadores / efectos:</strong> {{ armorPenaltyText(selectedCatalogArmor) }}</p><p v-if="inventoryError" class="error-banner" role="alert">{{ inventoryError }}</p></div><footer class="modal-actions"><button class="button button-primary" type="button" :disabled="armorCatalogLoading" @click="addSelectedCatalogArmor">{{ armorCatalogLoading ? 'Añadiendo…' : 'Añadir armadura' }}</button><button class="button button-quiet" type="button" @click="showCatalogArmorModal=false">Cancelar</button></footer></section></div>

    <div v-if="showCatalogPhysicalShieldModal && selectedCatalogPhysicalShield" class="modal-backdrop" @click.self="showCatalogPhysicalShieldModal=false"><section class="modal-card weapon-detail-modal" role="dialog" aria-modal="true" aria-labelledby="catalog-physical-shield-detail-title"><header class="modal-header"><div><p class="eyebrow accent">AÑADIR ESCUDO FÍSICO</p><h2 id="catalog-physical-shield-detail-title">{{ selectedCatalogPhysicalShield.name }}</h2><p class="modal-copy">Revisa sus valores y confirma la incorporación.</p></div><button class="modal-close" type="button" aria-label="Cerrar ventana" @click="showCatalogPhysicalShieldModal=false">×</button></header><div class="modal-body weapon-detail-body"><div class="weapon-detail-hero"><div class="weapon-detail-image shield-detail-image"><img v-if="selectedCatalogPhysicalShield.imageUrl" :src="selectedCatalogPhysicalShield.imageUrl" :alt="selectedCatalogPhysicalShield.name"><span v-else>🛡</span></div><dl class="weapon-detail-stats"><div><dt>RD</dt><dd>{{ selectedCatalogPhysicalShield.rd || '—' }}</dd></div><div><dt>Defensa</dt><dd>+{{ selectedCatalogPhysicalShield.defense }}</dd></div><div><dt>Movimiento</dt><dd>{{ selectedCatalogPhysicalShield.movement || '—' }}</dd></div></dl></div><p v-if="selectedCatalogPhysicalShield.description" class="weapon-detail-rule">{{ selectedCatalogPhysicalShield.description }}</p><p v-if="inventoryError" class="error-banner" role="alert">{{ inventoryError }}</p></div><footer class="modal-actions"><button class="button button-primary" type="button" :disabled="shieldCatalogLoading" @click="addSelectedCatalogPhysicalShield">{{ shieldCatalogLoading ? 'Añadiendo…' : 'Añadir escudo' }}</button><button class="button button-quiet" type="button" @click="showCatalogPhysicalShieldModal=false">Cancelar</button></footer></section></div>
