...
async function importArchive(event: Event) { const file = (event.target as HTMLInputElement).files?.[0]; if (!file) return; if (!confirm('Se reemplazarán los datos de juego de este personaje. Su retrato y permisos se conservarán. ¿Continuar?')) return; try { await api.importCharacterArchive(String(route.params.id), JSON.parse(await file.text())); await load(); } catch (e: any) { error.value = e?.message || 'No se pudo importar el archivo.'; } finally { if (archiveInput.value) archiveInput.value.value = ''; } }
